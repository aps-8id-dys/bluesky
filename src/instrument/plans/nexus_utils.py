...
def create_runtime_metadata_dict(det=None):
    ...
    runtime_updates = {
        ...
        "/entry/cycle": pv_registers.cycle_name.get(),
        "/entry/scan_number": 1,
        "/entry/start_time": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()),
        "/entry/end_time": time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()), # fixme later
    
        "/entry/instrument/DETECTOR/beam_center_x": 1024.0,
        "/entry/instrument/DETECTOR/beam_center_y": 1024.0,
        "/entry/instrument/DETECTOR/count_time":  det.cam.acquire_time.get(),
        "/entry/instrument/DETECTOR/frame_time": det.cam.acquire_period.get(),
        "/entry/instrument/DETECTOR/detector_name": det.name,
        "/entry/instrument/DETECTOR/distance": 4.0,
