...
from ..devices.huber_diffractometer import huber
...
def setup_eiger_int_rock(acq_time, acq_period, num_frames, file_name):
...
def eiger_acq_int_rock(acq_period=0.1, 
                       num_frame=10, 
                       att_level=None,
                       angle=None, 
                       process=True):
    ...
    angle_name = int(angle*1e3)

    filename = f"{header_name}_{sample_name}_a{att_level:04}_f{num_frame:03d}_th{angle_name:05d}"

    yield from setup_eiger_int_rock(acq_time, acq_period, num_frame, filename)
